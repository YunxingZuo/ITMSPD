...
class Element(object):
    def __init__(self, arg):
        ...
        if arg in elements_dict:
            ...
            self.__atomic_mass = elements_data[self.__atomic_number][3]
        else:
    ...
    def __eq__(self, other):
        if isinstance(other, Element):
            if self.__atomic_symbol == other.atomic_symbol:
                if self.__atomic_number == other.atomic_number:
                    if self.__atomic_mass == other.atomic_mass:
                        return True
        else:
            return False
