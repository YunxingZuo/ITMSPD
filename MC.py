...
def func(structure, element_valence, limit = 0, bond_dist = 1.8, radius = 0.05, criterion = 0.2, times = 200):
    error = bv_mean_error_criterion(structure, element_valence)
    print error
    if error < criterion or limit > times:
        return structure, error, limit
    spg_symbol = structure.get_spacegroup()
    spg = SpaceGroup(spg_symbol)
    lattice = structure.lattice_vectors
    indep_index = structure.get_independent_atoms()
    fitted_positions = []
    for index in indep_index:
        index_positions = structure.frac_positions[structure.get_identical_atoms_and_operations()[0] == index]
        ...
        for pos in index_positions:
            if spg.get_wyckoff_letter(pos) == special_positions(spg_symbol, mul, pos):
                ...
                break
    indep_wyck = structure.atomic_wyckoff_letters()[indep_index]
    indep_atoms = np.array(structure.atomic_symbols)[indep_index]
    satisfied = False
    while not satisfied:
        relaxed_positions = []
        for disturb in fitted_positions:
            relaxed_positions.append(disturb + np.array([uniform(-1, 1), uniform(-1, 1), uniform(-1, 1)]) * radius)
        relaxed_positions = np.array(relaxed_positions)
        new_structure, satisfied = structure_generation_for_monte_carlo(spg_symbol, lattice, indep_wyck, indep_atoms, relaxed_positions, min_bond_dist = bond_dist)
    limit += 1
    print limit
    new_error = bv_mean_error_criterion(new_structure, element_valence)
    if new_error < error:
        structure = new_structure
        return func(structure, element_valence, limit, bond_dist, radius, criterion, times)
    else:
...
def normal_distribution(x, miu, sigma):
    return 0.5 * np.exp(-(x - miu) ** 2 / (2 * sigma ** 2))
